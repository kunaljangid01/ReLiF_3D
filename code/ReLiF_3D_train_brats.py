...
from val_3D_brats import test_all_case
...
parser.add_argument('--exp', type=str,
                    default='BraTS/ReLiF_3D', help='experiment_name')
...
def _gauss_blur3d(x: torch.Tensor, k: int) -> torch.Tensor:
    # cheap separable-ish blur via repeated avg_pool3d; k must be odd
    if k <= 1:
        return x
    if k % 2 == 0:
        k += 1
...
def sobf_augment(x: torch.Tensor,
    ...
    # scale to target strengths
    M = 1.0 + a_mul * m_raw.tanh()            # multiplicative gain field
    A = a_add * a_orth.tanh()                 # additive bias field

    # apply fields
    y = x * M + A

    # robust clamp to original per-sample range
        ...
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                avg_metric = test_all_case(
                    model_label_convent, args.root_path, test_list="val.txt", num_classes=2, patch_size=args.patch_size,
